...
from data_multicolors import generate_data, Generator
...
class RolloutBaseline:
	...
	def _update_baseline(self, model, epoch):
		...
		# 10000 samples in the baseline dataset
	...
	def eval_all(self, dataset):
		...
		"""
		print("Evaluating baseline model on baseline dataset (eval_all)")
		if self.alpha < 1:
			return None
	...
	def epoch_callback(self, model, epoch):
		...
		min_candidate_val = candidate_vals.min()
		
		print(f'Epoch {self.cur_epoch} candidate mean {candidate_mean}, baseline mean {self.mean}, best_val {min_candidate_val}')

		if candidate_mean < self.mean:
			# TODO: change these onto cpu
		...
		if min_candidate_val < self.global_best_val:
			self.global_best_val = min_candidate_val
			print(f'Best val so far: {self.global_best_val}')
	...
	def rollout(self, model, dataset, batch = 1000, disable_tqdm = False):
		...
		dataloader = DataLoader(dataset, batch_size = batch, num_workers=3)
